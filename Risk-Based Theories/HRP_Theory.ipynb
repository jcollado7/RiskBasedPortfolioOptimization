...
for ticker in tickers:
    ...
    historic_price_data = yahoo_financials.get_historical_price_data(start_date='2022-01-03', end_date=today, time_interval='daily')
...
fechas = returns_data.index.tolist()
...
for ticker in tickers_1:
    yahoo_financials = YahooFinancials(ticker)
    historic_price_data = yahoo_financials.get_historical_price_data(start_date='2021-01-01', end_date=today, time_interval='daily')
    prices = historic_price_data[ticker]['prices']
    prices_df = pd.DataFrame(prices)
    prices_df = prices_df.set_index('formatted_date')
    prices_df.index.name = 'date'
...
# Perform hierarchical clustering on the matrix D
...
sns.heatmap(corr_matrix, cmap='Greens', ax=axs[0])
axs[0].set_title('Original Correlation Matrix')

# plot the reordered correlation matrix
sns.heatmap(corr_reordered, cmap='Greens', ax=axs[1])
