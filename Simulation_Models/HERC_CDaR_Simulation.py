...
for i in range(start_index, len(returns_data_sp)-2):
    ...
    cdar_results = {}
    for ticker in tickers:
        rt_col_name = f'rt_{ticker}'
        var_col_name = f'VaR_{ticker}'
        returns_data["cum_" + rt_col_name] = (returns_data[rt_col_name] + 1).cumprod()
        returns_data["cum_" + rt_col_name] = returns_data["cum_" + rt_col_name].fillna(0)  
        returns_data["dar_" + rt_col_name] = [float('nan')] * len(returns_data["cum_" + rt_col_name])
        returns_data["dar_" + rt_col_name][0] = 0

        for i in range(1, len(returns_data["cum_" + rt_col_name])):
            returns_data["dar_" + rt_col_name][i] = returns_data["cum_" + rt_col_name][i] - returns_data["cum_" + rt_col_name][i-1] + returns_data["dar_" + rt_col_name][i-1]
            if returns_data["dar_" + rt_col_name][i] > 0:
                returns_data["dar_" + rt_col_name][i] = 0
        returns_data["dar_" + rt_col_name] = -returns_data["dar_" + rt_col_name]
        returns_data["dar_2" + rt_col_name] = returns_data["dar_" + rt_col_name] + 1
        percentile = returns_data["dar_" + rt_col_name].quantile(0.99)
        dar = returns_data["dar_" + rt_col_name][returns_data["dar_" + rt_col_name] > percentile]

        cdar = dar.mean()
        cdar_results[ticker] = cdar
    stock_cDaR = pd.Series(cdar_results)
    stock_cDaR.rename(lambda x: f'rt_{x}', inplace=True)
    ...
    for i in range(optimal_k-1):
        if i > 0:
            ...
            sum_of_lists = [stock_cDaR.loc[['rt_' + asset for asset in assets_list]].sum() for assets_list in different_clusters]
            ...
            sum_of_lists = [stock_cDaR.loc[['rt_' + asset for asset in assets_list]].sum() for assets_list in clusters_list_1]
            total_sum_of_inverse = sum(1 / sum_of_list for sum_of_list in sum_of_lists)
            weights_clusters = [(1 / sum_of_list) / total_sum_of_inverse for sum_of_list in sum_of_lists]
...
#df.to_excel('C:\\--\--\--\HERC_CDaR_Sim.xlsx', index=False)
