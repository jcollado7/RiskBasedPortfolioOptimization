...
for i in range(start_index, len(returns_data_sp)-2):
    ...
    for i in range(optimal_k-1):
        if i > 0:
            ...
            weights_clusters = []
            for sublist in different_clusters:
                cov_cluster = cov_matrix.loc[sublist, sublist]
                weight = (1 / np.trace(cov_cluster)) / sum(1 / np.trace(cov_matrix.loc[sublist, sublist]) for sublist in different_clusters)
                weights_clusters.append(weight)   
            ...
            weights_clusters = []
            for sublist in clusters_list_1:
                cov_cluster = cov_matrix.loc[sublist, sublist]
                weight = (1 / np.trace(cov_cluster)) / sum(1 / np.trace(cov_matrix.loc[sublist, sublist]) for sublist in clusters_list_1)
                weights_clusters.append(weight)
...
#df.to_excel('C:\\--\--\--\HERC_Variance_Sim.xlsx', index=False)
